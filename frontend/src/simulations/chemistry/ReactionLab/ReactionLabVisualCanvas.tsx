...
const ReactionLabVisualCanvas = ({
  params,
  isRunning,
  onAnalyticsUpdate,
}: ReactionLabVisualCanvasProps) => {
  ...
  useEffect(() => {
    ...
    const level2 =
      params.reactionType === "decomposition"
        ? 0
        : Math.min(90, (params.reactant2Amount / 100) * 90);
    ...
    const result = ReactionLabEngine.getReaction(
      params.reactionType,
      reactant1,
      reactant2
    );
    ...
    onAnalyticsUpdate({
      ...
      productsFormed: 0,
      totalParticles:
        params.reactant1Amount +
        (params.reactionType === "decomposition" ? 0 : params.reactant2Amount),
      balancedEquation: result?.valid
        ? result.equation
        : t("reactionLab.analytics.selectValid"),
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (isRunning) {
        switch (state.phase) {
          case "pouring1":
            if (state.tube1Level > 3) {
              ...
              const firstPourTarget = Math.min(
                45,
                (params.reactant1Amount / 200) * 90
              );
              state.beakerLevel = Math.min(
                firstPourTarget,
                state.beakerLevel + pourRate * 0.5
              );
              state.pourStream.progress = Math.min(
                1,
                state.pourStream.progress + 0.04
              );
          ...
          case "pouring2":
            if (state.tube2Level > 3) {
              ...
              const mixRatio =
                1 - state.tube2Level / Math.max(1, initialLevel2);
        ...
        const progress =
          state.phase === "complete"
            ? 100
            : state.phase === "reacting"
            ? 50 + state.reactionProgress * 0.5
            : state.phase === "mixing"
            ? 30 + state.mixProgress * 0.2
            : state.phase === "pouring2"
            ? 15 + (1 - state.tube2Level / Math.max(1, initialLevel2)) * 15
            : state.phase === "pouring1"
            ? (1 - state.tube1Level / Math.max(1, initialLevel1)) * 15
            : 0;

        // Total input mass in grams
        const totalInputMass =
          params.reactant1Amount +
          (params.reactionType === "decomposition"
            ? 0
            : params.reactant2Amount);
        ...
        const moles2 =
          params.reactionType === "decomposition"
            ? 0
            : params.reactant2Amount / molarMass2;
        ...
        const limitingReactantCoeff =
          limitingReagent === reactant1 ? coeff1 : coeff2;
        const excessReactantCoeff =
          limitingReagent === reactant1 ? coeff2 : coeff1;
        const excessMolarMass =
          limitingReagent === reactant1 ? molarMass2 : molarMass1;

        // Mass of excess reagent consumed
        const excessMolesConsumed =
          (limitingMoles * excessReactantCoeff) / limitingReactantCoeff;
        ...
        const yieldEfficiency = 0.9 + (params.temperature / 200) * 0.08;
        const actualYield =
          (progress / 100) * theoreticalYield * yieldEfficiency;
        const percentYield =
          theoreticalYield > 0 ? (actualYield / theoreticalYield) * 100 : 0;

        // Reaction rate based on temperature and phase (g/s)
        const baseRate = (params.temperature / 100) * 2;
        const reactionRate =
          state.phase === "reacting"
            ? baseRate + Math.random() * 0.3
            : state.phase === "mixing"
            ? baseRate * 0.3
            : 0;

        // Products formed in GRAMS (what user expects to see)
        const productsFormedGrams =
          (progress / 100) * theoreticalYield * yieldEfficiency;
        ...
        onAnalyticsUpdate({
          ...
          limitingReagent:
            params.reactionType === "decomposition"
              ...
              : limitingReagent,
          excessReagent:
            params.reactionType === "decomposition" ? "" : excessReagent,
      ...
      const tube1Tilt =
        state.phase === "pouring1" && state.pourStream.active
      ...
      ctx.fillText(
        reactant1 || t("reactionLab.canvas.empty"),
        tube1X,
        tube1Y + tubeH + 30
      );
      ctx.font = "11px Arial";
      ...
      ctx.fillText(
        `${Math.round((state.tube1Level / 90) * 100)}g`,
        tube1X,
        tube1Y + tubeH + 45
      );
      ...
      if (params.reactionType !== "decomposition") {
        ...
        const tube2Tilt =
          state.phase === "pouring2" && state.pourStream.active
            ? -Math.min(50, state.pourStream.progress * 55) * (Math.PI / 180)
            : 0;
        ...
        ctx.textAlign = "center";
        ctx.fillText(
          reactant2 || t("reactionLab.canvas.empty"),
          tube2X,
          tube2Y + tubeH + 30
        );
        ctx.font = "11px Arial";
        ...
        // Tube level 90 = 100g, so current grams = (level / 90) * 100
        ctx.fillText(
          `${Math.round((state.tube2Level / 90) * 100)}g`,
          tube2X,
          tube2Y + tubeH + 45
        );
      }
      ...
      if (state.pourStream.active && state.pourStream.progress > 0) {
        ...
        const rotatedMouthX =
          mouthOffsetX * Math.cos(tiltAngle * tiltDirection) -
          mouthOffsetY * Math.sin(tiltAngle * tiltDirection);
        const rotatedMouthY =
          mouthOffsetX * Math.sin(tiltAngle * tiltDirection) +
          mouthOffsetY * Math.cos(tiltAngle * tiltDirection);
      ...
      // Beaker label
      let beakerLabel = t("reactionLab.canvas.reactionBeaker");
      if (state.phase === "complete" && result?.products) {
        beakerLabel = result.products.join(" + ");
      } else if (state.beakerLevel > 0) {
        if (state.phase === "reacting") {
          beakerLabel = t("reactionLab.status.reacting");
        } else if (state.phase === "mixing") {
          beakerLabel = t("reactionLab.status.mixing");
        }
      }
      ...
      ctx.font = "13px Arial";
      let statusColor = "#888";
      let statusText = t("reactionLab.canvas.pressStart");

      if (state.phase === "pouring1") {
        statusColor = "#fbbf24";
        statusText = t("reactionLab.status.pouring", { item: reactant1 });
      } else if (state.phase === "pouring2") {
        statusColor = "#fbbf24";
        statusText = t("reactionLab.status.pouring", { item: reactant2 });
      } else if (state.phase === "mixing") {
        statusColor = "#60a5fa";
        statusText = `${t("reactionLab.status.mixing")}... ${Math.round(
          state.mixProgress
        )}%`;
      } else if (state.phase === "reacting") {
        statusColor = "#f97316";
        statusText = `${t("reactionLab.status.reacting")}... ${Math.round(
          state.reactionProgress
        )}%`;
      } else if (state.phase === "complete") {
        statusColor = "#4ade80";
        statusText = t("reactionLab.status.complete");
      }
      ...
      if (state.phase === "reacting" || state.phase === "complete") {
        ...
        ctx.fillStyle =
          result?.energyChange === "exothermic" ? "#ef4444" : "#3b82f6";
        const energyText =
          result?.energyChange === "exothermic"
            ? t("reactionLab.canvas.heatReleased")
            : t("reactionLab.canvas.heatAbsorbed");
      ...
      ctx.textAlign = "center";
      ctx.fillText(t("reactionLab.canvas.visualLab"), width / 2, 28);
  ...
  }, [dimensions, isRunning, params, getReactants, onAnalyticsUpdate, t]);
